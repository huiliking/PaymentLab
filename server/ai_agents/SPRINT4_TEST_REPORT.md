...
**Run date:** 2026-07-28, follow-ups 2026-08-02
**Executed against:** `master` @ commit `726e780` (Sprint 4 changes; report itself was written before that commit landed)
**Environment:** Windows 11 dev machine (Phases 0–5), WSL2 Ubuntu (Phase 3 Linux verification), Flask dev server (`python app.py`), Vite dev client, curl + browser automation
**Procedure followed:** `SPRINT4_TEST_PLAN.md`, all 6 phases

## Result: PASS

All 12 validation criteria from `sprint4-handover.md` hold, and — as of the second 2026-08-02 follow-up below — both branches of the cross-process lock have now been verified for real (not just the Windows one). No open gaps remain locally; only the exact Render multi-worker topology is still unverified in production (see Phase 6).

**2026-08-02 follow-up #1:** an external review of this report (via claude.chat) flagged that Phase 3.1's first pass didn't actually exercise the code path it claimed to test, that the Phase 6 deferral read as optional rather than required, that the single-shared-admin-key trade-off wasn't written down anywhere, and that the test plan's own inline comment still had the "400ish" wording even though the pass-criteria line had been corrected. All four addressed: 3.1 was re-run against a genuinely threaded instance and now confirms the intended lock-contention message (see Phase 3 below); Phase 6 language was strengthened to "hard gate"; the admin-key limitation is now `KNOWN_ISSUES.md` #4; the test plan's remaining "400ish" comment was fixed. WSL2 was suggested for locally exercising the untested `fcntl` branch — not installed as part of that follow-up since it requires enabling a Windows feature and a restart; deferred to the user.

**2026-08-02 follow-up #2:** the user installed WSL2 themselves and ran `_test_concurrent_lock.py` under WSL2 Ubuntu — the real `fcntl.flock` branch, not a Windows fallback. **Passed**, same criteria as the Windows run: both concurrent writes survived, valid JSON, lock file created. See the updated Phase 3 table and `KNOWN_ISSUES.md` #5 (now marked resolved). One hiccup along the way, unrelated to the lock itself: creating the venv directly on the Windows-mounted path (`/mnt/c/...`) failed with an `ensurepip` error — a known WSL/DrvFs symlink issue, worked around by creating the venv on WSL's native filesystem (`~/venv-linux`) instead and pointing `pip install -r` at the Windows-mounted `requirements.txt`.
...
| 3.2 (Windows, `msvcrt`) | Real cross-process concurrency (`_test_concurrent_lock.py`) | **PASS** — run twice for confirmation. Two independent OS processes, synchronized via `multiprocessing.Barrier`, called `propose_tool()` simultaneously for distinct dummy tools. Both writes survived, `registry.json` stayed valid JSON, `.lock` sidecar file was created. Exit code 0 both runs. **This is the test that actually proves the KNOWN_ISSUES.md #1 lost-update race is fixed** — reproduces the exact scenario (separate processes, each with their own in-memory `ToolRegistry`) that a same-process test cannot. |
| 3.2 (Linux, `fcntl.flock`, 2026-08-02) | Same script, same procedure, run for real under WSL2 Ubuntu | **PASS** — `[PASS] both dummy tools present after concurrent writes`, `[PASS] lock sidecar file created`, cleanup succeeded. This exercises the actual POSIX branch of `_CrossProcessLock` (`sys.platform != "win32"` → `fcntl.flock`), i.e. the exact code path Render runs — not a Windows fallback. Closes `KNOWN_ISSUES.md` #5. One setup snag along the way: creating the venv directly on the `/mnt/c/...` Windows-mounted path failed (`ensurepip` error — a known WSL/DrvFs limitation); worked around by creating the venv on WSL's native filesystem (`~/venv-linux`) and installing from the Windows-mounted `requirements.txt` instead. A stray zero-byte `registry.json.lock` was left behind after the run (the script cleans up dummy tools but not its own lock file, same as the Windows runs) — removed manually. `registry.json` also picked up an LF/CRLF-only diff from being rewritten by Linux Python; confirmed zero content difference via `git diff` and discarded with `git checkout --`. |
| 3.3 | `.lock` sidecar file gitignored | **PASS** — confirmed present in `.gitignore` (`*.lock`), file removed from working tree after each test run (both Windows and WSL2 runs) |
...
**Update 2026-08-02:** the `fcntl` branch is no longer untested — see Phase 3's WSL2 row above and `KNOWN_ISSUES.md` #5 (resolved). What's below is what's still genuinely open, now a lighter-weight sanity check rather than a hard gate, since the real lock code has already been proven correct under genuine concurrent Linux processes.

- `PAYMENTLAB_ADMIN_KEY` needs its own value set as a Render env var (the dev `.env` value must not be reused there). Still open — no way to verify locally.
- The exact **Render deployment topology** (gunicorn's multi-worker-process model, behind Render's own scheduling) hasn't been reproduced — WSL2 proved the `fcntl.flock` code path works under genuine concurrent Linux processes, which is the main thing that mattered, but it's still one VM rather than gunicorn's actual worker model. Optional extra confidence: run against two genuinely concurrent gunicorn workers directly.
- **Curl-against-a-live-URL remains a weak test on its own**, if anyone's tempted to use it as the verification: two near-simultaneous `curl` calls against a deployed Render URL can get serialized by network latency or dyno scheduling before they ever reach the lock, so a "pass" there is weak positive evidence at best — not a substitute for the WSL2/gunicorn-worker style tests above.
